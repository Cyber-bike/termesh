//! v2.0 accept loop: the part of `termy-agent run` that serves a control end
...
//! - every bi-stream on the admitted connection is one terminal session:
//!   doc 8.2 handshake, then frames pumped both ways until either side
//!   closes,
...
use crate::termstream::{ClosePayload, ErrorPayload, Frame, FrameDecoder, OpenedPayload, ShellEventPayload};
    ...
    loop {
        match connection.accept_bi().await {
            Ok((send, recv)) => {
                ...
                let options = Arc::clone(&options);
                tokio::spawn(async move {
                    serve_terminal_stream(send, recv, table, options).await;
                });
            }
            Err(e) => {
...
/// One `termy/terminal/1` bi-stream = one session (doc 8.2).
async fn serve_terminal_stream(
    ...
    // 1. Handshake: the first frame must be `open`.
    let open = match tokio::time::timeout(
        OPEN_HANDSHAKE_TIMEOUT,
        read_frame(&mut recv, &mut decoder),
    )
    .await
    {
        Ok(Ok(Some(Frame::Open(payload)))) => payload,
        Ok(Ok(Some(_))) => {
            send_error(&mut send, "PROTOCOL_ERROR: expected an open frame").await;
            return;
        }
        Ok(Ok(None)) | Ok(Err(_)) => return,
        Err(_) => {
            send_error(&mut send, "PROTOCOL_ERROR: open handshake timed out").await;
            return;
        }
    };
...
mod tests {
    ...
    use crate::termstream::{OpenPayload, ResizePayload};
    ...
    async fn start_agent(max_sessions: usize) -> (Endpoint, String, tokio::task::JoinHandle<()>) {
    ...
    fn a_full_terminal_session_works_over_loopback_quic() {
        ...
        runtime.block_on(async {
            let (agent, code, serve_task) = start_agent(4).await;
    ...
    fn the_session_cap_refuses_with_session_limit_reached() {
        let runtime = tokio::runtime::Runtime::new().unwrap();
        runtime.block_on(async {
            let (agent, code, serve_task) = start_agent(1).await;
            let (controller, connection) = connect_controller(&code).await;
