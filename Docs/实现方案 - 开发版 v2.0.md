...
| 项目 | 内容 |
| --- | --- |
| 输入依据 | [需求文档 v2.0](需求文档 v2.0.md)、[需求文档 - 用户版 v2.0](需求文档 - 用户版 v2.0.md) |
...
> **关于新增依赖的核实状态**：本文档引入的 `iroh` / `iroh-blobs` 相关结论基于 2026-07-30 的公开文档核实（网络搜索），用于确认方向性可行（QUIC 端到端加密、Relay 不可解密、支持多路复用流、JS/TS 官方绑定自 2025 年起暂停积极维护等）。**具体版本号、API 细节和 MSRV 必须在阶段 A 开工时重新对照 crates.io / docs.rs 实测锁定**，本文档中出现的版本号仅作为起点参考，不作为最终锁定依据；这与 V1 文档 1.1 节"精确锁定依赖"的要求一致。
...
4. 控制端可同时对多台已配对且在线的设备建立连接；对同一台设备可创建多个并发终端会话，各自独立的输入、输出、resize、关闭。
...
10. 本地终端和本地拖拽行为不变；每设备并发终端数、控制端并发设备数均有可配置软上限。
...
flowchart LR
    subgraph A[Windows 控制端]
        ...
        BRIDGE[termy-bridge：本地伴生进程]
    end

    subgraph RLY[可选：iroh Relay（仅打洞失败时经由）]
        RELAY[只转发加密报文，无法解密]
    end
    ...
    UI --> NOTE
    NOTE --> VAULT
    MULTI <-->|本机回环：控制/数据| BRIDGE
    NOTE <-->|本机回环| BRIDGE
    BRIDGE <-->|优先：QUIC 直连| AGENT1
    BRIDGE <-->|优先：QUIC 直连| AGENT2
    BRIDGE -.->|降级| RELAY
    RELAY -.-> AGENT1
    RELAY -.-> AGENT2
    AGENT1 --> PTY1 --> SHELL1
...
| 进程 | 职责 | 与 V1 的关系 |
| --- | --- | --- |
| Obsidian 插件（TypeScript） | UI、xterm.js 绑定、多设备/多终端状态管理、笔记收集 | 沿用 V1 UI 层，替换远程数据来源为本机 `termy-bridge` |
| `termy-bridge`（新增，Rust） | 持有 iroh Endpoint，管理与各目标设备的连接、承载终端字节流与文件传输的实际网络 I/O，向插件暴露本机回环控制通道 | 新增进程；替代 V1 中"插件直接持有 WSS 客户端连云端"的角色 |
...
`termy-bridge` 的引入原因见第 6.2 节：iroh 官方 JS/TS 绑定自 2025 年 2 月起被官方暂停积极更新，团队公开说明尚未达到"在非 Rust 语言中开箱即用"的体验，因此不适合作为插件的直接生产依赖；插件已有"启动并管理一个本机 Rust 进程”的成熟模式（现有 `ServerManager` 管理本地 PTY server），`termy-bridge` 复用同一模式。
...
4. 单个 iroh 连接原生支持任意数量的并发 QUIC 流且互不产生队头阻塞；据此设计：**一条到某设备的连接可以承载该设备下的全部并发终端会话**，新建终端 = 在既有连接上开一条新流，天然满足"单设备多终端”；控制端持有多条到不同 `NodeId` 的连接，天然满足"多设备并发”。
...
9. v2.0 默认使用 iroh 官方托管中继（供打洞失败时兜底）；是否自建中继列入待确认事项（见[需求文档 v2.0](需求文档 v2.0.md)第 12 节）。
...
sequenceDiagram
    ...
    participant Bridge as 控制端 termy-bridge
    participant Plugin as Obsidian 插件

    Agent->>Agent: 首次运行生成身份，之后每次启动打印连接码
    User->>Plugin: 打开"添加设备"，粘贴连接码
    Plugin->>Bridge: addDevice(ticket)
    Bridge->>Bridge: 解析 NodeTicket
    Bridge->>Agent: 尝试建立 iroh 连接（优先直连）
    Agent-->>Bridge: 连接建立成功
    Bridge-->>Plugin: 设备已添加，显示在线
```

- 连接过程不涉及任何服务端参与的"注册/绑定"业务调用；`Bridge` 与 `Agent` 直接互相验证 QUIC/TLS 层身份（对端 `NodeId` 是否与票据一致），无需额外应用层鉴权握手。
- 插件本地设备列表条目：`{ name, ticketDigest, nodeId, lastConnectedAt, lastKnownOnline }`，不落库到任何服务端。
...
| 插件 ↔ 本机进程 | `ws`（沿用 V1，用途从"连云端 WSS"收窄为"连本机 `termy-bridge`”） | 本机回环，不再需要 TLS（回环通道），但需要本机进程间的最小身份校验（见 6.2） |
| 新增本地进程 | `termy-bridge`（Rust，见 6.2） | 持有 iroh Endpoint |

插件侧原有的 `src/services/remote/authClient.ts`、`deviceClient.ts`、`relayClient.ts` 在 v2.0 中废弃或大幅改写：不再需要账号登录与云端设备查询；`relayClient.ts` 的职责收窄为"与本机 `termy-bridge` 通信”，可重命名为 `bridgeClient.ts`。

### 6.2 新增本地伴生进程 `termy-bridge`

**为什么不直接在插件里用 iroh 的 JS/TS 绑定**：iroh 官方从 2025 年 2 月起暂停了对 Kotlin/Python/Swift/JavaScript 这些 FFI 绑定的持续更新，官方明确表示尚未达到"在非 Rust 语言中开箱即用"的体验目标；直接依赖一个官方已声明暂缓维护的绑定，对生产插件是不可接受的风险。相反，Termy 已经有"插件启动并管理一个本机 Rust 进程"的成熟先例（现有 `ServerManager` 之于本地 PTY server），`termy-bridge` 复用完全相同的模式：原生 Rust 依赖 `iroh`/`iroh-blobs`，稳定性和上游支持力度都远高于绕道 FFI。

职责：

1. 持有一个 iroh `Endpoint`，代表控制端在 iroh 网络中的身份（该身份也是持久化的，避免每次启动都用新身份导致目标端把它当作"新客户端”）。
2. 管理"已配对设备"列表的本地持久化（JSON 文件，权限限制为仅当前用户可读写）。
3. 对插件暴露一个本机回环控制通道（建议沿用 `ws`，监听 `127.0.0.1` 随机端口，端口和一次性握手令牌通过插件启动进程时的标准输入/命令行参数传递，避免其他本机进程猜测连接，这与现有本地 PTY server 的自保护方式一致）。
4. 把"添加设备”“新建终端”“终端输入输出”“发起传输”等来自插件的请求，转换为对相应 `Agent NodeId` 的 iroh 流操作。
...
interface RemoteSessionHandle {
    ...
    sessionId: string;  // termy-bridge 分配的本地会话标识
...
- 连接（到某个控制端 `NodeId`）整体断开时，该连接下的所有会话一并终止；同一 Agent 与另一个控制端的连接不受影响。
...
- Agent 长期运行，等待入站 iroh 连接（作为 iroh 的"可被连接方”），同时其 `Endpoint` 会主动向home relay 保持在线心跳（iroh 内建行为）。
- 与某个控制端的连接断开后，Agent 不主动重连该控制端（应由控制端发起重连），沿用 V1"断开即终止会话，重连不恢复会话"的取舍。
...
| iroh-blobs 内建 ALPN | 文件内容拉取 | 由 `iroh-blobs` 库自身定义，Agent 作为请求方向 `termy-bridge`（作为提供方）拉取内容 |
...
1. 控制端把待发送文件通过 `iroh-blobs` 在 `termy-bridge` 侧计算/导入为内容寻址的 blob（得到 BLAKE3 哈希）。
2. 控制端在 `termy/transfer/1` 流上发送上表清单（含 `sessionId`，用于 7.6 的落盘目标解析）。
3. Agent 校验清单（数量、大小、路径合法性，规则沿用 V1 §10.3），通过后作为请求方经 `iroh-blobs` 从 `termy-bridge` 拉取每个 blob 内容，边拉取边做 BLAKE3 增量校验。
...
sequenceDiagram
    ...
    participant Bridge as termy-bridge
    participant Agent as termy-agent
    participant PTY as PowerShell / Bash

    UI->>Bridge: 新建终端(deviceId)
    Bridge->>Agent: 打开 termy/terminal/1 流，发送 open{cols,rows}
    Agent->>PTY: 创建 PTY
    Agent-->>Bridge: opened{sessionId, shell}
    Bridge-->>UI: 终端已连接
    ...
    Agent-->>Bridge: shellEvent{cwd=...}（用户 cd 后触发）
    Bridge-->>UI: 更新该终端已知 cwd（用于下次拖拽落盘）

    UI->>Bridge: 关闭该终端
    Bridge->>Agent: close 帧
    Agent->>PTY: 终止进程树
```

多台设备、多个终端的数据流彼此独立，均为上图的并发实例，互不共享状态（除 `termy-bridge` 内部的连接复用）。
...
sequenceDiagram
    ...
    View->>Bridge: 发起传输(sessionId, entries)
    Bridge->>Bridge: 计算/导入 BLAKE3 内容寻址 blob
    Bridge->>Agent: termy/transfer/1 清单(sessionId, entries+hash)
    Agent->>Agent: 校验数量/大小/路径；解析落盘根目录（7.6）
    Agent->>Bridge: 经 iroh-blobs 按哈希拉取内容
    Agent->>Disk: 边校验边落盘
    Agent-->>Bridge: 传输结果
    Bridge-->>View: 转发结果
...
- 默认使用 iroh 官方托管中继作为打洞失败时的兜底路径；中继只依据 `NodeId` 转发已加密的 QUIC 报文，不具备解密能力（基于 iroh 官方文档确认，见文首核实说明）。
- 是否需要自建 `iroh-relay`（供无法访问公共中继的网络环境，或希望完全自主可控网络路径的用户）列入待确认事项，不阻塞 v2.0 首个可用版本。
- 控制端与目标端在建立连接时会各自尝试直连候选地址与经中继两条路径，采用"先打洞、失败退化"的既有 iroh 行为，本项目不需要自行实现该逻辑。
...
6. `termy-bridge` 的本机回环控制通道必须做最小身份校验（如一次性握手令牌），避免同机其他进程冒充插件操纵已建立的远程连接。
7. Agent 与 `termy-bridge` 都执行路径校验，Agent 校验结果为最终依据（沿用 V1 §13）。
...
- 集成 `iroh`：`termy-bridge` 与 `termy-agent` 各自生成/持久化身份，完成点对点连接的最小闭环（无终端、无文件，只验证连上）。
- 实现连接码的生成、展示、解析、`rotate-identity`。
- 实现插件 <-> `termy-bridge` 的本机回环通道与最小身份校验。
...
| 层级 | v2.0 必测内容 |
| --- | --- |
| 插件/`termy-bridge` 单元测试 | 连接码解析、设备列表本地持久化、多会话状态管理、拖拽分流、cwd 展示更新 |
| Agent 单元测试 | 身份生成/rotate、会话表并发管理、进程树终止、cwd 跟踪更新、路径安全（含逃逸检测基准目录切换）、BLAKE3 校验失败处理 |
| 协议集成测试 | 终端流开/输入/输出/resize/close 全链路、传输握手与 iroh-blobs 拉取全链路、错误码触发 |
| 多设备/多终端专项 | 同设备多终端互不影响、多设备并发互不影响、达到 `maxConcurrentSessions` 后的拒绝与提示 |
| 网络穿透专项 | 直连成功路径、直连失败降级中继路径、直连和中继都失败的提示 |
...
6. 本地终端和本地拖拽无行为回归。
7. 安全检查（端到端加密、路径防逃逸、本机回环身份校验、日志脱敏）通过。
8. 文档齐全：Agent 安装、CLI、连接码使用说明、隐私披露（能力凭证模型的用户须知）、v2.0 限制说明。
...
4. 每设备并发终端数、控制端并发设备数存在可配置软上限，超出会被拒绝而非排队等待。
5. 默认依赖 iroh 官方托管中继作为打洞失败兜底；自建中继暂未纳入 v2.0。
...
| 并发模型 | 每账号一条控制连接，每 Agent 一个远程 PTY | 控制端多设备并发连接，单设备多终端并发会话 |
...
对已经按 V1 方案落地的部分，迁移影响主要集中在：移除云端 Auth/Device/Relay 服务及其依赖的 SQLite 数据、移除插件侧账号相关设置项与登录页、Agent 网络层从 WSS 客户端改为 iroh Endpoint、新增 `termy-bridge` 进程。PTY 封装、shell 集成事件产出、Vault 附件收集规则、Windows/Ubuntu 双端支持等能力可直接复用，不需要重新实现。
