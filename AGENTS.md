...
- **Include client-side telemetry.** No analytics SDKs, no usage pings, no error reporting endpoints, no anonymous-id beacons, no "first-run phone home". The README already commits to "no telemetry" — that commitment is contractual. The only outbound network calls allowed are: downloading the matching `termy-server` binary from GitHub Releases; downloading the fixed-version platform-specific iroh `.node` runtime from unpkg, jsDelivr, or GitHub Releases (native downloads are disabled by offline mode and verified with SHA-256); the local-only WebSocket connections to the PTY backend and the Claude Code IDE bridge; and the optional AI launcher update check (`registry.npmjs.org` for Claude Code / Codex CLI, `api.github.com` for OpenCode) which ships **disabled by default** behind the `checkAiLauncherUpdates` setting and is suppressed by offline mode regardless of the toggle. The setting must remain off-by-default; flipping the default is a breaking policy change that requires a corresponding README update.
