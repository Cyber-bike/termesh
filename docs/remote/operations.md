...
| `TERMY_ACCESS_TOKEN_TTL_SECS` | 访问令牌有效期，默认 `900`（方案 §6.2 的 15 分钟），可填 60–86400 |

**关于 15 分钟这个默认值**：方案定的是 15 分钟且没有 refresh token。但插件端不保存密码，所以到期就得重新手输一次——实际使用中每刻钟被打断一次，等于不可用。控制 WSS 只在握手时校验令牌，连接建立后不受影响，所以真正被卡住的是设备轮询和新建连接。

个人单用户部署建议设成 `43200`（12 小时），代价是令牌万一泄露有效期更长。多用户或对外提供服务的部署应该保留默认值。
