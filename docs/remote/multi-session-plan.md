...
> **历史文档，已被 v2.0 实际实现取代。** 多会话已经落地（`agent/src/session_table.rs`），但走的不是本文档设想的"不设上限 + 每会话流控"，而是`maxConcurrentSessions`（默认 8）软上限、超出即拒绝（`SESSION_LIMIT_REACHED`）不排队、没有单独的流控协议面。当前实际设计见《Docs/实现方案 - 开发版 v2.0.md》和 [plugin-handover.md](plugin-handover.md)。保留本文档只作为设计过程记录。
